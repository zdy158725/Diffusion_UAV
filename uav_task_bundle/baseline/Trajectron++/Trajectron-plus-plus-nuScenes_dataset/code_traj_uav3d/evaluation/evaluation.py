...
def compute_obs_violations(predicted_trajs, map):
    ...
    pred_trajs_map = map.to_map_points(predicted_trajs[..., :2].reshape((-1, 2)))
...
def _flatten_metric_values(metric_values):
    flattened = []
    for value in metric_values:
        value_array = np.asarray(value, dtype=np.float64)
        if value_array.size == 0:
            continue
        flattened.extend(value_array.reshape(-1).tolist())
    return np.asarray(flattened, dtype=np.float64)


def summarize_batch_errors(batch_errors_list):
    if not batch_errors_list:
        return {}
    ...
    for node_type in batch_errors_list[0].keys():
        ...
        for metric in batch_errors_list[0][node_type].keys():
            ...
            metric_values = _flatten_metric_values(metric_batch_error)
        ...
        if node_summary:
            summary[node_type.name] = node_summary
...
def log_batch_errors(batch_errors_list, log_writer, namespace, curr_iter, bar_plot=[], box_plot=[]):
    ...
    summary = summarize_batch_errors(batch_errors_list)
    if not batch_errors_list:
        return summary
    ...
    for node_type in batch_errors_list[0].keys():
        for metric in batch_errors_list[0][node_type].keys():
            ...
            metric_values = _flatten_metric_values(metric_batch_error)
            if metric_values.size > 0:
                metric_summary = summary.get(node_type.name, {}).get(metric, {})
                log_writer.add_histogram(f"{node_type.name}/{namespace}/{metric}", metric_values, curr_iter)
                log_writer.add_scalar(f"{node_type.name}/{namespace}/{metric}_mean", metric_summary.get('mean', float(np.mean(metric_values))), curr_iter)
                log_writer.add_scalar(f"{node_type.name}/{namespace}/{metric}_median", metric_summary.get('median', float(np.median(metric_values))), curr_iter)
                ...
                if metric in bar_plot:
                    ...
                    if visualization_module and getattr(visualization_module, 'visualization_utils', None) is not None:
                        kde_barplot_fig, ax = plt.subplots(figsize=(5, 5))
                        visualization_module.visualization_utils.plot_barplots(ax, pd, 'dataset', metric)
                ...
                if metric in box_plot:
                    ...
                    if visualization_module and getattr(visualization_module, 'visualization_utils', None) is not None:
                        ...
                        visualization_module.visualization_utils.plot_boxplots(ax, mse_fde_pd, 'dataset', metric)

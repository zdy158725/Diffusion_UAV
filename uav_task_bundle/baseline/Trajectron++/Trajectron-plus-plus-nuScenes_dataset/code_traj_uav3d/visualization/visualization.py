...
def plot_trajectories_3d(ax,
    ...
    for node in histories_dict:
        ...
        ax.plot(history[:, 0], history[:, 1], history[:, 2], 'k--', linewidth=1.0)

        for sample_num in range(predictions.shape[0]):
            ax.plot(predictions[sample_num, :, 0],
                    predictions[sample_num, :, 1],
                    predictions[sample_num, :, 2],
                    color=cmap[node.type.value],
                    linewidth=line_width,
                    alpha=line_alpha)

        ax.plot(future[:, 0], future[:, 1], future[:, 2], color='black', linestyle='--', linewidth=1.2)
        ax.scatter(history[-1, 0], history[-1, 1], history[-1, 2], color='g', edgecolors='k', s=25, zorder=3)
...
def visualize_prediction_3d(ax,
                            prediction_output_dict,
                            dt,
                            max_hl,
                            ph,
    ...
    plot_trajectories_3d(ax, prediction_dict, histories_dict, futures_dict, *kwargs)

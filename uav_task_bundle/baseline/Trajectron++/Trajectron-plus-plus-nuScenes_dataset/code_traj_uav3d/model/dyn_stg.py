...
class SpatioTemporalGraphCVAEModel(object):
    ...
    @staticmethod
    def _expand_current_position_for_predictions(current_position, prediction_ndim):
        # Predictions are shaped like [samples_z, samples_gmm, batch, ph, pred_dim].
        # We want current_position [batch, pred_dim] to broadcast over sample axes and time:
        # [1, 1, batch, 1, pred_dim] for 5D, [1, batch, 1, pred_dim] for 4D, etc.
        expanded = np.expand_dims(current_position, axis=-2)
        while expanded.ndim < prediction_ndim:
            expanded = np.expand_dims(expanded, axis=0)
        return expanded

    def _prepare_node_inputs_and_labels(self, inputs, labels, node_type, vehicle_type):
        uniform_t = self.max_hl
        position_dim = len(self.state[node_type.name]['position'])

        inputs = np.array(inputs)
        labels = np.array(labels)
        current_position = inputs[:, uniform_t, :position_dim].copy()
        ...
        # Make prediction targets relative to the current position.
        labels_rel = labels - current_position[:, np.newaxis, :]

        # Build a doubled state tensor: relative motion features + absolute current position context.
        ...
        std = np.tile(std, 2)
        inputs_aug = np.tile(inputs, 2)
        inputs_aug[..., self.state_length[node_type.name]:self.state_length[node_type.name] + position_dim] = current_position[:, np.newaxis, :]
        inputs_st = self.env.standardize(inputs_aug,
                                         self.state[node_type.name],
                                         mean=rel_state,
                                         std=std,
                                         node_type=node_type)
        labels_st = self.env.standardize(labels_rel,
                                         self.pred_state[node_type.name],
                                         mean=np.zeros((labels_rel.shape[0], 1, labels_rel.shape[2]),
                                                       dtype=labels_rel.dtype),
                                         node_type=node_type)

        if vehicle_type is not None and node_type == vehicle_type:
            inputs_aug[..., 0:2] = pos_org

        return inputs_aug, labels, labels_rel, labels_st, current_position, inputs_st

    def _unstandardize_relative_predictions(self, predictions, node_type):
        # Prediction targets are standardized relative positions, so they must be
        # unstandardized with zero mean before we add the current position back.
        zero_mean = np.zeros(predictions.shape[-1], dtype=predictions.dtype)
        return self.env.unstandardize(
            predictions,
            self.pred_state[node_type.name],
            node_type,
            mean=zero_mean,
    ...
    def train_loss(self, scene, timesteps, max_nodes=None):
        vehicle_type = getattr(self.env.NodeType, 'VEHICLE', None)
        losses = dict()
        ...
        for node_type in self.env.NodeType:
            ...
            uniform_t = self.max_hl
            inputs, labels, _, labels_st, _, inputs_st = self._prepare_node_inputs_and_labels(
                inputs, labels, node_type, vehicle_type
            )
    ...
    def predict(self,
        ...
        for node_type in self.env.NodeType:
            ...
            uniform_t = self.max_hl
            inputs, labels, _, labels_st, current_position, inputs_st = self._prepare_node_inputs_and_labels(
                inputs, labels, node_type, vehicle_type
            )
            ...
            predictions_uns = self._unstandardize_relative_predictions(
                predictions.cpu().detach().numpy(),
                node_type,
            )
            predictions_uns = predictions_uns + self._expand_current_position_for_predictions(
                current_position, predictions_uns.ndim
            )

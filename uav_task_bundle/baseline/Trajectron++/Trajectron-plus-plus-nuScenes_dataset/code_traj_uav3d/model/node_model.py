...
class MultimodalGenerativeCVAE(object):
    def __init__(self,
        ...
        if self.hyperparams['incl_robot_node']:
            if 'VEHICLE' not in self.state:
                raise ValueError("incl_robot_node=True requires a VEHICLE entry in hyperparams['state'].")
            self.robot_state_length = int(np.sum([len(entity_dims) for entity_dims in self.state['VEHICLE'].values()]))
        else:
            self.robot_state_length = 0
    ...
    def create_graphical_model(self, edge_types):
        ...
        self.add_submodule(self.node_type + '/decoder/proj_to_GMM_log_sigmas',
                           model_if_absent=nn.Linear(self.hyperparams['dec_rnn_dim'],
                                                     self.hyperparams['GMM_components'] * self.pred_state_length))
        self.add_submodule(self.node_type + '/decoder/proj_to_GMM_tril',
                           model_if_absent=nn.Linear(self.hyperparams['dec_rnn_dim'],
    ...
    def encode_edge(self,
        ...
        for i, timestep_in_scene in enumerate(timesteps_in_scene):  # Get neighbors for timestep in batch
            ...
            for node in connected_nodes[i]:
                ...
                std[0:3] = self.env.attention_radius[edge_type]
    ...
    def project_to_GMM_params(self, tensor):
        ...
        tril = self.node_modules[self.node_type + '/decoder/proj_to_GMM_tril'](tensor)
        return log_pis, mus, log_sigmas, tril
        ...
        if mode in [ModeKeys.TRAIN, ModeKeys.EVAL]:
            ...
            if self.hyperparams['sample_model_during_dec'] and mode == ModeKeys.TRAIN:
                ...
                for j in range(ph):
                    ...
                    log_pi_t, mu_t, log_sigma_t, tril_t = self.project_to_GMM_params(h_state)
                    y_t = GMM3D(log_pi_t, mu_t, log_sigma_t, tril_t, self.pred_state_length, self.device,
                ...
                log_pis, mus, log_sigmas, tril = self.project_to_GMM_params(outputs)
            ...
            if self.hyperparams['log_histograms'] and self.log_writer is not None:
                ...
                self.log_writer.add_histogram('%s/%s' % (str(self.node_type), 'GMM_log_sigmas'), log_sigmas,
                                              self.curr_iter)
                self.log_writer.add_histogram('%s/%s' % (str(self.node_type), 'GMM_tril'), tril, self.curr_iter)
            ...
            if most_likely_gmm:
                raise NotImplementedError("most_likely_gmm is disabled in code_traj_uav3d.")

            log_pis, mus, log_sigmas, tril, y = [], [], [], [], []
            for j in range(ph):
                h_state = cell(input_, state)
                log_pi_t, mu_t, log_sigma_t, tril_t = self.project_to_GMM_params(h_state)

                gmm = GMM3D(log_pi_t, mu_t, log_sigma_t, tril_t, self.pred_state_length, self.device,
                            self.hyperparams['log_sigma_min'],
